...
`tools/list` at `https://gateway.pipeworx.io/thegamesdb/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/thegamesdb_search_games \
  -H 'Content-Type: application/json' \
  -d '{"name":"zelda","limit":10}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/thegamesdb_search_games`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "thegamesdb": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-thegamesdb"]
    }
  }
}
```
